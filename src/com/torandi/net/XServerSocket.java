...
public class XServerSocket extends ServerSocket {
    ...
    private XStreamListener xparent=null;
    ...
    public XServerSocket(XStreamListener xparent) throws IOException{
        super();
        this.xparent = xparent;
    }
    
    public XServerSocket(int port,XStreamListener xparent) throws IOException{
        super();
        this.xparent = xparent;
        multiListen("localhost",port);
    }
    
    public XServerSocket(String bind, int port,XStreamListener xparent) throws IOException{
        super();
        this.xparent = xparent;
        multiListen(bind, port);
    }
    ...
    public void setParent(XStreamListener xparent) {
        this.xparent = xparent;
    } 
    
    public boolean multiListen(String bind, int port) throws IOException{
        if(xparent != null) {
    ...
    public void run() {
        while(!Thread.interrupted()) {
            try {
                ...
                if(sck.isOutputOn() && run) {
                    System.out.println("XSocketServer: Stopped to listen for new clients: ");
                    e.printStackTrace();
                }
                break;
            }
