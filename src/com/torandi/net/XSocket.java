...
public class XSocket extends Socket{
    ...
    private XStreamListener xlistener = null;
     ...
     * xlistener (and inits xstreams)
     * @param host
     * @param port
     */
    public XSocket(String host, int port,XStreamListener xstream) {
        xlistener=xstream;
        connect(host,port);
        initXStreamReciver();
    }
    ...
    void dataRecived(String data,ByteBuffer b) {
        if(! (ignoreEmpty && data.trim().equals(""))) {
            xlistener.dataRecived(data,b,this);
        }
    }
    ...
    public void setListener(XStreamListener xlistener) {
    ...
    void reconnect() {
        ...
        if(!closeCalled) {
            ...
            if(xlistener != null) {
                if(!server) {
                    xlistener.connectionClosed(this);
                    xlistener.reconnect(this);
                } else {
                    xlistener.connectionClosed(this);
                }
            } else {
